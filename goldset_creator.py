...
data = [
    {
        ...
        "expected_sql": "SELECT c.city, SUM(s.amount) AS total_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE s.month >= strftime('%Y-%m', date('now', '-6 months')) GROUP BY c.city ORDER BY total_amount DESC LIMIT 1000;"
    },
    {
    ...
    },
    {
        ...
        "expected_sql": "SELECT c.city, c.segment, SUM(s.amount) AS total_spending FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE c.credit_tier IN (4, 5) GROUP BY c.city, c.segment ORDER BY total_spending DESC LIMIT 1000;"
    },
    {
        "question": "2023 yılında toplam alışveriş sayısı nedir?",
        "expected_sql": "SELECT SUM(s.purchases) AS total_purchases FROM sales s WHERE s.month LIKE '2023-%' LIMIT 1000;"
    },
    {
        "question": "Antalya’daki müşterilerin ortalama geliri ne kadar?",
        "expected_sql": "SELECT AVG(income) AS avg_income FROM customers WHERE city = 'Antalya' LIMIT 1000;"
    },
    {
        "question": "Her segmentin toplam harcama tutarı nedir?",
        "expected_sql": "SELECT c.segment, SUM(s.amount) AS total_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id GROUP BY c.segment LIMIT 1000;"
    },
    {
        "question": "Aylara göre toplam ciro değişimi nedir?",
        "expected_sql": "SELECT s.month, SUM(s.amount) AS total_amount FROM sales s GROUP BY s.month ORDER BY s.month LIMIT 1000;"
    },
    {
        "question": "2024 yılında İstanbul'daki toplam alışveriş sayısı kaç?",
        "expected_sql": "SELECT SUM(s.purchases) AS total_purchases FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE c.city = 'Istanbul' AND s.month LIKE '2024-%' LIMIT 1000;"
    },
    {
        "question": "En yüksek ortalama harcamaya sahip 5 şehir nedir?",
        "expected_sql": "SELECT c.city, AVG(s.amount) AS avg_spending FROM sales s JOIN customers c ON s.customer_id = c.customer_id GROUP BY c.city ORDER BY avg_spending DESC LIMIT 1000;"
    },
    {
    ...
    },
    {
        ...
        "expected_sql": "SELECT AVG(s.amount) AS avg_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE c.city = 'Ankara' AND c.credit_tier = 5 LIMIT 1000;"
    },
    {
    ...
    },
    {
        ...
        "expected_sql": "SELECT c.segment, SUM(s.amount) AS total_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE s.month LIKE '2024-%' GROUP BY c.segment LIMIT 1000;"
    },
    {
        "question": "Geliri en yüksek 10 müşterinin şehir ve segment bilgisi nedir?",
        "expected_sql": "SELECT city, segment, income FROM customers ORDER BY income DESC LIMIT 1000;"
    },
    {
        "question": "Tier 1 müşterilerin toplam alışveriş adedi nedir?",
        "expected_sql": "SELECT SUM(s.purchases) AS total_purchases FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE c.credit_tier = 1 LIMIT 1000;"
    },
    {
        "question": "2023 yılında toplam kaç müşteri alışveriş yaptı?",
        "expected_sql": "SELECT COUNT(DISTINCT s.customer_id) AS total_customers FROM sales s WHERE s.month LIKE '2023-%' LIMIT 1000;"
    },
    {
        "question": "İzmir'deki müşterilerin ortalama kredi skoru kaçtır?",
        "expected_sql": "SELECT AVG(credit_tier) AS avg_credit_tier FROM customers WHERE city = 'Izmir' LIMIT 1000;"
    },
    {
        "question": "Her kredi skoru için ortalama harcama ne kadar?",
        "expected_sql": "SELECT c.credit_tier, AVG(s.amount) AS avg_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id GROUP BY c.credit_tier ORDER BY c.credit_tier LIMIT 1000;"
    },
    {
    ...
    },
    {
        ...
        "expected_sql": "SELECT s.month, c.segment, SUM(s.amount) AS total_amount FROM sales s JOIN customers c ON s.customer_id = c.customer_id WHERE s.month LIKE '2024-%' GROUP BY s.month, c.segment ORDER BY s.month LIMIT 1000;"
    },
    {
        "question": "100 bin TL altı gelire sahip müşterilerin segment dağılımı nedir?",
        "expected_sql": "SELECT segment, COUNT(*) AS customer_count FROM customers WHERE income < 100000 GROUP BY segment ORDER BY customer_count DESC LIMIT 1000;"
    },
    {
