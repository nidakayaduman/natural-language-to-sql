...
data = [
    ...
    # 5
    {
        ...
        "expected_sql": """
            SELECT c.city, c.segment, SUM(s.amount) AS total_spending
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            WHERE c.credit_tier IN (4, 5)
    ...
    # 6
    {
        ...
        "expected_sql": """
            SELECT SUM(s.purchases) AS total_purchases
            FROM sales s
            WHERE s.month LIKE '2023-%'
            LIMIT 1000;
        """
    },
    # 7
    ...
    # 10
    {
        ...
        "expected_sql": """
            ...
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            WHERE c.city = 'Istanbul' AND s.month LIKE '2024-%'
            LIMIT 1000;
        """
    },
    # 11
    {
        ...
        "expected_sql": """
            SELECT c.city, AVG(s.amount) AS avg_spending
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            GROUP BY c.city
    ...
    # 13
    {
        ...
        "expected_sql": """
            SELECT AVG(s.amount) AS avg_amount
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            WHERE c.city = 'Ankara' AND c.credit_tier = 5
            LIMIT 1000;
        """
    },
    # 14
    ...
    # 15
    {
        ...
        "expected_sql": """
            SELECT s.month, AVG(s.purchases) AS avg_purchases
            FROM sales s
            GROUP BY s.month
            ORDER BY s.month
            LIMIT 1000;
    ...
    # 20
    {
        ...
        "expected_sql": """
            SELECT AVG(credit_tier) AS avg_credit_tier
            FROM customers
            WHERE city = 'Izmir'
            LIMIT 1000;
        """
    ...
    # 21
    {
        ...
        "expected_sql": """
            SELECT c.credit_tier, AVG(s.amount) AS avg_amount
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            GROUP BY c.credit_tier
            ORDER BY c.credit_tier
            LIMIT 1000;
        """
    },
    # 22
    ...
    # 23
    {
        ...
        "expected_sql": """
            SELECT s.month, c.segment, SUM(s.amount) AS total_amount
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            WHERE s.month LIKE '2024-%'
            GROUP BY s.month, c.segment
            ORDER BY s.month
            LIMIT 1000;
        """
    },
    ...
    # 25
    {
        ...
        "expected_sql": """
            SELECT customer_id, SUM(purchases) AS total_purchases, SUM(amount) AS total_amount
            FROM sales
    ...
    # 26
    {
        "question": "2024 yılında en çok harcama yapan ilk 5 müşteri kim?",
        "expected_sql": """
    ...
    # 27
    {
        "question": "Her şehirde en çok harcama yapan müşteri kim?",
        "expected_sql": """
            SELECT c.city, s.customer_id, SUM(s.amount) AS total_spent
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
    ...
    # 28
    {
        "question": "2023-2024 yıllarında toplam ciro karşılaştırması nedir?",
        "expected_sql": """
            SELECT SUBSTR(s.month, 1, 4) AS year, SUM(s.amount) AS total_amount
            FROM sales s
            GROUP BY year
            ORDER BY year
            LIMIT 1000;
    ...
    # 29
    {
        "question": "Her segment için müşteri başına ortalama harcama nedir?",
        "expected_sql": """
            SELECT c.segment, AVG(s.amount) AS avg_spent
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            GROUP BY c.segment
            LIMIT 1000;
    ...
    # 30
    {
        "question": "Aynı şehirde yaşayan Tier 5 müşterilerin toplam harcaması nedir?",
        "expected_sql": """
            SELECT c.city, SUM(s.amount) AS total_spent
            FROM sales s
            JOIN customers c ON s.customer_id = c.customer_id
            WHERE c.credit_tier = 5
            GROUP BY c.city
            ORDER BY total_spent DESC
            LIMIT 1000;
        """
...
df.to_csv(os.path.join(DATA_DIR, "goldset_v2.csv"), index=False)
